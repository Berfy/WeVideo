...
public class ActivityManager {
    ...
    public void popActivity(List<Class<?>> classes) {
        if (null != activityStack) {
            for (int i = 0; i < activityStack.size(); i++) {
                Activity activity = activityStack.elementAt(i);
                for (Class<?> className : classes) {
                    if (activityStack.elementAt(i).getClass().equals(className)) {
                        LogUtil.e("关闭Activity", className + "");
    ...
    public String getLastActivityTitle() {
        ...
        if (size > 0) {
            return activityStack.get(size - 1).getTitle() + "";
        }
        return "";
    }
